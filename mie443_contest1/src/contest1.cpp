...
void odomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    tf::getYaw(msg->pose.pose.orientation);ROS_INFO("Position: (%f,%f) Orientation:%frad or%fdegrees.", posX, posY,yaw,RAD2DEG(yaw));
}
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok() && secondsElapsed <= 900) {
        ros::spinOnce();
        //
        // check bumper message
        // ROS_INFO("bumperLEFT: %d", bumper[0]);
        // ROS_INFO("bumperCENTRE: %d", bumper[1]);
